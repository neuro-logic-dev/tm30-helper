...
**Что изменилось (ADR-0021, хелпер 2.5.0).** Раньше логин/пароль портала ехали
внутри самой deep link в base64 — то есть секреты лежали в URL, который попадает
в историю браузера, в логи ОС и в переписку. Ссылка тира `v: 4` не несёт
**никаких** учётных данных вообще: в ней только `{api_base, token,
focus_filing_id?}`. Сам worklist (уже вместе с парами логин/пароль) хелпер теперь
забирает с API по HTTPS, предъявляя операторский токен как
`Authorization: Bearer` — ровно та схема, которая раньше была записана здесь как
намерение. Токен хранится в `tm30-state.json` в `userData` (обычный файл, без
шифрования), строки worklist'а живут только в памяти и на диск не пишутся.

**Чего это НЕ закрывает — сознательно.** Ужесточение учётных данных не сделано;
это отдельная, названная итерация («модуль сначала, потом секьюрность»):

- пароли вилл по-прежнему передаются — теперь в теле ответа worklist'а, а не в
  ссылке. Транспорт стал лучше (HTTPS + авторизация вместо URL), состав секретов —
  нет;
- у токена нет срока жизни и нет отзыва по машине. Утёкшая строка читает очередь
  всей компании, включая эти пароли, и может пометить подачу `submitted`;
- единственный рычаг отзыва до той итерации — ротация `JWT_SECRET` на весь парк
  сразу.

Пересмотреть, когда: подключается оператор вне текущего доверенного круга, либо
машина потеряна/расшарена. Плановое решение — отзываемые токены на установку,
обмен при паринге и хранение через `safeStorage`.
